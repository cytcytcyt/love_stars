<template>
    <el-row class="publish-container">
        ...
        <div class="form-item">
            <input readonly type="text" placeholder="请上传图片" v-model="this.uploadContent.file.name" autocomplete="off">
            <input id="fileInput" type="file" name="filename" accept="image/gif,image/jpeg,image/jpg,image/png,image/svg" @change="getFile($event)" size="30">
            <label for="fileInput" class="file-btn">上传图片</label>
        </div>
        <el-input placeholder="请选标签" >
            <template slot="prepend"><i class="el-icon-price-tag"></i></template>
            <el-tag slot="prefix"
    ...
    export default {
        ...
        methods: {
            handleClose(tag) {
                ...
                this.uploadContent.tag.splice(this.uploadContent.tag.indexOf(tag.name), 1)
            },
            handleChoose(tag) {
                if (tag.type === 'info') {
                    tag.type = 'warning'
            ...
            publishUploadContent(){
                ...
                axios.post('http://localhost:8088/api/publishPicture',formData,config).then(res => {
                    if(res.status === 200){
                        this.$message({
                            showClose: true,
                            message:"发表成功啦！",

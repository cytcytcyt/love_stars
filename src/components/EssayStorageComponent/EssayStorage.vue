<template>
    <div class="EScontainer">
        ...
        <el-row>
            <el-col :span="12">
                <el-row v-for="(i,index) in articlesList1.slice((curPage-1)*(pageSize/2),curPage*(pageSize/2))" :key="index">
                <el-avatar class="left-avatar" size="large" src="https://cube.elemecdn.com/3/7c/3ea6beec64369c2642b92c6726f1epng.png"></el-avatar>
                <div class="left-bubble" >
                    <div @click="handleTextInfo(i)">
                        <span class="article-title">{{i.title}}</span><br>
                        <span class="article-content">{{i.article}}</span>
                    </div>
                    <el-divider></el-divider>
                    <el-row class="icon-buttons">
                        <i :class="i.onOrOff ? 'el-icon-star-on' : 'el-icon-star-off'" @click="handleClickStar(i)"></i>&nbsp;&nbsp;&nbsp;
            ...
            <el-col :span="12">
                <el-row  v-for="(i,index) in articlesList2.slice((curPage-1)*(pageSize/2),curPage*(pageSize/2))" :key="index">
                ...
                <el-avatar class="right-avatar" size="large" src="https://cube.elemecdn.com/3/7c/3ea6beec64369c2642b92c6726f1epng.png"></el-avatar>
                </el-row>
            </el-col>
    ...
    export default {
        ...
        watch:{
            selectedTags(val) {
                ...
                axios.get('http://localhost:8088/api/selectArticles'+'?'+tags.toString())
                    .then(response => {
        ...
        mounted(){
            axios.get('http://localhost:8088/api/selectArticles')
                .then(response => {
                    console.log('response',response)
                    let index =0
                    let resp = response
